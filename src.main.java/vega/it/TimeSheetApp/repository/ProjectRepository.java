...
public interface ProjectRepository extends JpaRepository<Project, Integer> {
	...
	@Query(value = "SELECT p.description, p.deleted, p.finished, p.client_id,p.project_id, p.project_name, tm.team_member_id, tm.username FROM project p INNER JOIN team_members tm ON p.team_member_id = tm.team_member_id where tm.username = :teamMemberUsername",nativeQuery = true )
